...
ゲームエンジン（Unity/Unreal Engine）の物理演算機能を使用せず、運動方程式の数値積分を自前で実装することで、物理挙動の基礎原理への理解と実装力を示すことを目的としています。
...
本プログラムでは、FortranによるHPC（ハイパフォーマンスコンピューティング）の経験を活かし、Pythonにおいても計算効率と物理的正確性を意識した実装を行っています。
